...
for start_time in info_sheet["D"]:
    if type(start_time.value) == datetime :
        record_start_time.append(start_time.value.strftime("%H:%M:%S"))
        record_date.append(start_time.value.strftime("%Y-%m-%d"))
for end_time in info_sheet["E"] :
    if type(end_time.value) == datetime :
...
print(f"type record date {record_date}")
# /////////////////////////////////////////////////////// A CHANGER //////////////////////////////////////////////
dom_UL = "Droit"
...
for data_idx in range(0, min(len(dom_counts), len(non_dom_counts))) : 
    ...
    # Record's end : Data reaching the max_time (20h) or dead battery 
        ...
        if therapy == "HABIT" or therapy == "Vie quotidienne":
